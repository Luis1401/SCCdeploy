-- MySQL Script generated by MySQL Workbench
-- Tue Dec  4 20:41:52 2018
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='TRADITIONAL,ALLOW_INVALID_DATES';

-- -----------------------------------------------------
-- Schema dbcombustible
-- -----------------------------------------------------
DROP SCHEMA IF EXISTS `dbcombustible` ;

-- -----------------------------------------------------
-- Schema dbcombustible
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `dbcombustible` DEFAULT CHARACTER SET utf8 ;
SHOW WARNINGS;
-- -----------------------------------------------------
-- Schema new_schema1
-- -----------------------------------------------------
SHOW WARNINGS;
USE `dbcombustible` ;

-- -----------------------------------------------------
-- Table `vehiculos`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `vehiculos` ;

SHOW WARNINGS;
CREATE TABLE IF NOT EXISTS `vehiculos` (
  `cod_interno` VARCHAR(20) NOT NULL,
  `nro_placa` VARCHAR(10) NOT NULL,
  `marca` VARCHAR(30) NOT NULL,
  `modelo` VARCHAR(30) NOT NULL,
  `nro_chasis` VARCHAR(20) NOT NULL,
  `tipo_combustible` VARCHAR(20) NOT NULL,
  `cilindrada` DECIMAL(10,0) NOT NULL,
  `categoria` VARCHAR(30) NOT NULL,
  `cap_tanque` INT(11) NOT NULL,
  `bsisa` VARCHAR(15) NOT NULL,
  PRIMARY KEY (`cod_interno`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1;

SHOW WARNINGS;

-- -----------------------------------------------------
-- Table `funcionarios`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `funcionarios` ;

SHOW WARNINGS;
CREATE TABLE IF NOT EXISTS `funcionarios` (
  `id_funcionario` INT(11) NOT NULL,
  `ci` INT(11) NOT NULL,
  `nombres` VARCHAR(20) NOT NULL,
  `paterno` VARCHAR(20) NOT NULL,
  `materno` VARCHAR(20) NOT NULL,
  `fecha_nac` DATE NOT NULL,
  `lugar_nac` VARCHAR(30) NOT NULL,
  `telf_empleado` INT(11) NOT NULL,
  `telf_celular` INT(10) NOT NULL,
  `categoria_licencia` VARCHAR(5) NOT NULL,
  `cargo` VARCHAR(20) NOT NULL,
  `departamento` VARCHAR(30) NOT NULL,
  PRIMARY KEY (`id_funcionario`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1;

SHOW WARNINGS;

-- -----------------------------------------------------
-- Table `asignacion_vehiculos`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `asignacion_vehiculos` ;

SHOW WARNINGS;
CREATE TABLE IF NOT EXISTS `asignacion_vehiculos` (
  `id_asignacion` INT(11) NOT NULL,
  `fecha_asignacion` DATE NOT NULL,
  `autorizado_por` VARCHAR(20) NOT NULL,
  `estado_vehiculo` VARCHAR(30) NOT NULL,
  `kilometraje` INT(5) NOT NULL,
  `observaciones` VARCHAR(30) NOT NULL,
  `vehiculos_cod_interno` VARCHAR(20) NOT NULL,
  `funcionario_id_funcionario` INT(11) NOT NULL,
  PRIMARY KEY (`id_asignacion`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1;

SHOW WARNINGS;

-- -----------------------------------------------------
-- Table `carga_comb`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `carga_comb` ;

SHOW WARNINGS;
CREATE TABLE IF NOT EXISTS `carga_comb` (
  `id_carga` INT(10) NOT NULL,
  `fecha_carga` DATETIME NOT NULL,
  `cod_interno` VARCHAR(20) NULL,
  `ci_funcionario` INT(10) UNSIGNED NOT NULL,
  `tipo_comb` VARCHAR(20) NOT NULL,
  `cant_carga` VARCHAR(20) NOT NULL,
  `costo_carga` DECIMAL(10,0) NOT NULL,
  `precinto_ant` VARCHAR(10) NOT NULL,
  `precinto_act` VARCHAR(10) NOT NULL,
  `kilometraje` INT(10) NOT NULL,
  `nro_vale` INT(11) NOT NULL,
  `tipo_carga` VARCHAR(10) NOT NULL,
  `nro_factura` INT(10) NULL,
  `nro_autorizacion` VARCHAR(20) NULL,
  `cod_control` VARCHAR(20) NULL,
  `vehiculos_cod_interno` VARCHAR(20) NOT NULL,
  `funcionario_id_funcionario` INT(11) NOT NULL,
  PRIMARY KEY (`id_carga`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1;

SHOW WARNINGS;

-- -----------------------------------------------------
-- Table `compra_combustible`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `compra_combustible` ;

SHOW WARNINGS;
CREATE TABLE IF NOT EXISTS `compra_combustible` (
  `tipo_comb` VARCHAR(20) NOT NULL,
  `fecha_compra` INT(11) NOT NULL,
  `cant_combustible` INT(11) NOT NULL,
  `costo_combustible` INT(11) NOT NULL,
  `nro_factura` INT(10) NOT NULL,
  `nro_autorizacion` VARCHAR(20) NOT NULL,
  `cod_control` VARCHAR(20) NOT NULL,
  `razon_social` VARCHAR(30) NOT NULL,
  `nro_nit` VARCHAR(20) NOT NULL)
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1;

SHOW WARNINGS;

-- -----------------------------------------------------
-- Table `compra_precintos`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `compra_precintos` ;

SHOW WARNINGS;
CREATE TABLE IF NOT EXISTS `compra_precintos` (
  `fecha_compra` DATE NOT NULL,
  `cantidad_precinto` INT(11) NOT NULL,
  `costo_precinto` INT(11) NOT NULL,
  `nro_factura` INT(10) NOT NULL,
  `nro_autorizacion` VARCHAR(20) NOT NULL,
  `cod_control` VARCHAR(20) NOT NULL,
  `razon_social` VARCHAR(30) NOT NULL,
  `nro_nit` INT(15) NOT NULL,
  `nro_inicial` INT(11) NOT NULL,
  `nro_final` INT(11) NOT NULL)
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1;

SHOW WARNINGS;

-- -----------------------------------------------------
-- Table `usuarios`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `usuarios` ;

SHOW WARNINGS;
CREATE TABLE IF NOT EXISTS `usuarios` (
  `id_usuario` VARCHAR(45) NOT NULL,
  `login` VARCHAR(15) NOT NULL,
  `clave` VARCHAR(10) NOT NULL,
  `rol` VARCHAR(30) NOT NULL,
  `nombre_usuario` VARCHAR(30) NOT NULL,
  `fecha_creacion` DATE NOT NULL,
  `estado` VARCHAR(30) NOT NULL,
  PRIMARY KEY (`id_usuario`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1;

SHOW WARNINGS;

-- -----------------------------------------------------
-- Table `permisos`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `permisos` ;

SHOW WARNINGS;
CREATE TABLE IF NOT EXISTS `permisos` (
  `id` INT NOT NULL,
  `nombre` VARCHAR(30) NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;

SHOW WARNINGS;

-- -----------------------------------------------------
-- Table `funcionariopermisos`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `funcionariopermisos` ;

SHOW WARNINGS;
CREATE TABLE IF NOT EXISTS `funcionariopermisos` (
  `idfuncionariopermisos` INT NOT NULL,
  `funcionarios_id_funcionario` INT(11) NOT NULL,
  `permisos_id` INT NOT NULL,
  PRIMARY KEY (`idfuncionariopermisos`, `funcionarios_id_funcionario`, `permisos_id`))
ENGINE = InnoDB;

SHOW WARNINGS;

SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
